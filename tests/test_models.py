...
class TestProductModel(unittest.TestCase):
    ...
    def test_update_a_product(self):
        ...
        product.id = None
        product.create()
        # print("Created %s", product)

        # Assert that the ID of the product object is not None after calling the create() method.
        self.assertIsNotNone(product.id)
        product.description = "test text"

        initial_id = product.id

        product.update()
        self.assertEqual(product.id, initial_id)
        self.assertEqual(product.description, "test text")

        products = Product.all()
        self.assertEqual(len(products), 1)
        self.assertEqual(products[0].id, initial_id)
        self.assertEqual(products[0].description, "test text")
